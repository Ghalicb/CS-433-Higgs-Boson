...
def cross_validation_SGD(y, tx, K, B, gamma, seed):
  ...
  for k in range(K):
    ...
    f = lambda a: a[ np.concatenate( [ k_indices[i] for i in range(len(k_indices)) if i != k ] )][:]
    tx_test, y_test = map(f, (tx, y))

    # Train
    # w, loss_tr = SGD(y_train, tx_train, gamma, B)
    # Test
    # How do I get the loss? It's in SGD
